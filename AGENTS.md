...
1. **XDG cache**: `${XDG_CACHE_HOME:-~/.cache}/pf/<spdx|includes>/` — one shared slot every pf-* binary reads and writes, so a purge in one clears the cache the others also read.
